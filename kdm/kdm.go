...
func Kdm() {
	...
	for {
		...
		for t := 0; t < 30; t++ {
			getStatus()
			logger.Info.Printf("Фаза %d Время %d ЖМ %d ", Status.Phase, Status.Time, Status.Blink)
		...
		for phase := 1; phase <= 3; phase++ {
			...
			for t := 0; t < 30; t++ {
				getStatus()
				logger.Info.Printf("Фаза %d Время %d Ру %d ", Status.Phase, Status.Time, Status.PhaseRU)
				time.Sleep(1 * time.Second)
			}
		}
		stopRUandBlink()
		for t := 0; t < 90; t++ {
			getStatus()
			logger.Info.Printf("Фаза %d Время %d ", Status.Phase, Status.Time)
			time.Sleep(1 * time.Second)
		}
	}
}
